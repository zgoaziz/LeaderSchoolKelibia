...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  const allowed = ["status","issue_date","expiry_date","notes","formation_name","student_name"];
  const update: Record<string, unknown> = {};
  for (const k of allowed) if (k in body) update[k] = body[k];
  const { data, error } = await db().from("certificates").update(update).eq("id", id).select("*, formations(id,title), students(id,first_name,last_name)").single();
